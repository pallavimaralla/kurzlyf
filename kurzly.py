...
def main():
    if request.method == "POST":
        url_received = request.form["ol"]
        #If url exsists in database
        found_url = Urls.query.filter_by(long=url_received).first()

        if found_url:
            #If ound in database,then return the same url
...
def display_short_url(url):
    return render_template('kurzlylink.html', short_url_display=url)

@app.route('/<short_url>')
def redirection(short_url):
    #Redirection of the short URL to the original URL
